...
{
public:
	int CreateDXObjects( IDirect3DDevice8 *pDevice);
	void Render(int texturing=1, int material=1, DWORD hVertexShader=D3DFVF_CUSTOMVERTEX_3ds,  IDirect3DDevice8 *pDevice=NULL);
...
public:
	...
	C3ds( char *file_name, int filter=2, int faceNormal=0, int shadowObject=0);
	C3ds();
	virtual ~C3ds();
