...
int SphereVN::Init(int del_uhol_x, int del_uhol_y, float size, IDirect3DDevice8 *pd)
{
	if(pvb!=NULL) pvb->Release();
	...
	void* p;
	if( FAILED( pvb->Lock( 0, del_x*2*(del_y+1)*sizeof(zSphereVN_CUSTOMVERTEX), (BYTE**)&p, 0 ) ) )return 0;
	ComputeSphere( del_x, del_y, (zSphereVN_CUSTOMVERTEX*)p, size);
	pvb->Unlock();
	return 1;
}

void SphereVN::Render( DWORD hVertexShader, IDirect3DDevice8 *pd)
{
