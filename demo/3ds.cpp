...
void C3ds::ComputeNormals()
{
	for(int i=0; i<this->objects.size(); i++)
	{
		...
		int count;
		for(int j=0; j<obj->numOfVerts; j++)			// Go through all of the vertices
		{
			count=0;
...
void C3ds::Render(int texturing, int material, DWORD hVertexShader, IDirect3DDevice8 *pDevice)
{
	int j;
	...
	for(int i=0; i<dxobjs.size(); i++)
	{
		...
		if(mat!=NULL)
		{
			...
			case 1:
				...
				{
					textures[mat->texureId].SetTexture(0,pDevice);
		...
		pDevice->SetStreamSource( 0, dxobjs[i].pvb, sizeof(z3ds_CUSTOMVERTEX) );
		
		pDevice->SetVertexShader( hVertexShader );

		for( j=0; j<dxobjs[i].count-21000; j+=21000)
			pDevice->DrawPrimitive( D3DPT_TRIANGLELIST, 3*j, 21000 );
		pDevice->DrawPrimitive( D3DPT_TRIANGLELIST, 3*j, dxobjs[i].count-j );

		if(texturing==1) pDevice->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_DISABLE );
	}
...
int C3ds::CreateDXObjects( IDirect3DDevice8 *pDevice)
{
	z3ds_DXobj dxobj;
