...
{
public:
	void Render( DWORD hVertexShader = zSphereVN_D3DFVF_CUSTOMVERTEX, IDirect3DDevice8 *pd=NULL);
	void ComputeSphere(int del_uhol_x, int del_uhol_y, zSphereVN_CUSTOMVERTEX *v, float size=1.0f);
	int Init( int del_uhol_x, int del_uhol_y, float size=1.0f, IDirect3DDevice8 *pd=NULL);
